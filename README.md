...
基于深度学习的垃圾分类 AI 助手,支持**拍照识别 + 语音/文字搜索 + 智能分类**。系统采用 YOLO26 目标检测模型,结合多模态融合推理(YOLO26 + SAHI 切片推理 + 双层级联分类),覆盖中国标准的四大垃圾类别(厨余垃圾、可回收物、其他垃圾、有害垃圾),共 40 种常见垃圾物品。
...
- **真实投放点数据**: 集成高德地图 POI 周边搜索 API，获取用户位置附近真实的垃圾桶/回收站/废品回收点
- **高精度定位**: 浏览器 Geolocation API + `enableHighAccuracy` + `maximumAge: 0`，每次获取最新位置
- **坐标系转换**: GCJ-02(火星坐标系) ↔ WGS-84(GPS坐标系) 双向转换，确保地图标注精准
- **多数据源融合**: 高德 POI(主) + OpenStreetMap Overpass(备选) + 本地数据库(参考)
- **智能过滤**: 自动排除奢侈品回收、二手车等不相关 POI，只保留真实垃圾投放点
...
| GET | `/api/map/amap-nearby` | 高德 POI 周边搜索(真实垃圾桶/回收站) |
| GET | `/api/map/nearby` | 附近投放点(本地数据库) |
...
1. **YOLO26 40 类模型** — 最高优先级,直接检测 40 种常见垃圾
...
- ✨ 集成高德地图 POI 周边搜索 API，获取真实垃圾桶/回收站位置数据
- ✨ 新增 GCJ-02 ↔ WGS-84 坐标系双向转换，确保地图标注精准
- ✨ 多数据源融合：高德 POI(主) + OpenStreetMap Overpass(备选) + 本地数据库(参考)
- ✨ 智能 POI 过滤：自动排除奢侈品回收等不相关商家
- 🔧 修复定位精度：`maximumAge` 从 300000ms 改为 0，每次获取最新 GPS 位置
- 🔧 修复定位超时：`timeout` 从 10000ms 增加到 15000ms
