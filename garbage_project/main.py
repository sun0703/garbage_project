...
# ==================== 模糊搜索引擎 ====================
class SearchEngine:
    """基于FuzzyWuzzy的模糊搜索引擎（增强版：拼音搜索 + 同义词扩展）"""

    def __init__(self, vocab_path: str):
        self.vocab: list[dict] = []
        self.vocab_labels: list[str] = []
        self.pinyin_index: dict[str, list[dict]] = {}
    ...
    def _load_vocab(self, vocab_path: str) -> None:
        ...
        try:
            ...
            logger.info("词库加载成功: %d 条记录, 拼音索引 %d 条, 同义词索引 %d 条",
                        len(self.vocab), len(self.pinyin_index), len(self.alias_index))
        except Exception as e:
    ...
    def _build_pinyin_index(self) -> None:
        ...
        for item in self.vocab:
            ...
            py_key = self.text_to_pinyin(label)
            if py_key:
                self.pinyin_index.setdefault(py_key, []).append(item)
            for alias in item.get("aliases", []):
                alias_py = self.text_to_pinyin(alias)
                if alias_py:
                    self.pinyin_index.setdefault(alias_py, []).append(item)
    ...
    def _build_alias_index(self) -> None:
        ...
        for item in self.vocab:
            label = item.get("label", "")
            self.alias_index.setdefault(label, []).append(item)
            for alias in item.get("aliases", []):
                self.alias_index.setdefault(alias, []).append(item)

    def _is_pinyin_query(self, query: str) -> bool:
        """判断查询是否为拼音（纯小写英文字母）"""
        return bool(query) and query.isalpha() and query.isascii() and query.islower()

    def search(self, query: str, top_k: int = 3) -> list[dict]:
        """执行模糊搜索（增强版：拼音+同义词+FuzzyWuzzy）"""
        ...
        if self._is_pinyin_query(query):
            pinyin_results = self._search_by_pinyin(query)
            for item in pinyin_results:
                label = item.get("label", "")
                if label not in seen_labels:
                    seen_labels.add(label)
                    results.append({**item, "similarity_score": 100, "match_type": "pinyin", "match_label": label})

        alias_results = self._search_by_alias(query)
        for item in alias_results:
            label = item.get("label", "")
            if label not in seen_labels:
                seen_labels.add(label)
                results.append({**item, "similarity_score": 100, "match_type": "alias", "match_label": label})

        raw_results = fuzz_process.extract(query, self.vocab_labels, limit=top_k * 2)
        for label, score in raw_results:
            if label not in seen_labels and score >= 40:
                item = next((v for v in self.vocab if v["label"] == label), None)
                if item:
                    seen_labels.add(label)
                    results.append({**item, "similarity_score": score, "match_type": "fuzzy", "match_label": label})
    ...
    def _search_by_pinyin(self, query: str) -> list[dict]:
        """拼音搜索：支持前缀匹配和完全匹配"""
        matched = []
        exact = self.pinyin_index.get(query, [])
        matched.extend(exact)

        if len(query) >= 2:
            for py_key, items in self.pinyin_index.items():
                if py_key != query and py_key.startswith(query):
                    for item in items:
    ...
    def _search_by_alias(self, query: str) -> list[dict]:
        """同义词搜索：精确匹配别名"""
        matched = []
        exact = self.alias_index.get(query, [])
        matched.extend(exact)

        if len(query) >= 2:
            for alias, items in self.alias_index.items():
                if alias != query and query in alias:
                    for item in items:
                        if item not in matched:
                            matched.append(item)
    ...
    def suggest(self, query: str, top_k: int = 8) -> list[dict]:
        ...
        if self._is_pinyin_query(query):
            for py_key, items in self.pinyin_index.items():
                if py_key.startswith(query) or query in py_key:
                    for item in items:
                        label = item.get("label", "")
        ...
        for item in self.vocab:
            ...
            if query in label or any(query in a for a in aliases):
                if label not in seen:
                    seen.add(label)
                    suggestions.append({
                        "label": label,
...
async def search_enhanced(
    ...
    """增强搜索接口：拼音+同义词+FuzzyWuzzy，返回更多结果和匹配类型"""
    if not search_engine or not search_engine.vocab:
        return JSONResponse(
            status_code=503,
            content={
    ...
    results = search_engine.search(query.strip(), top_k=top_k)
    is_pinyin = search_engine._is_pinyin_query(query.strip())
    return JSONResponse(
        content={
            "success": True,
            "query": query,
            "is_pinyin_query": is_pinyin,
            "results": results,
            "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ"),
        }
    )
...
async def search_suggest(
    ...
    suggestions = search_engine.suggest(query.strip(), top_k=top_k)
    return JSONResponse(
        content={
            "success": True,
            "query": query,
...
async def voice_to_text(audio: UploadFile = File(...)) -> JSONResponse:
    """
    语音识别接口（MediaRecorder降级方案）
    接收前端上传的音频文件，返回识别的文字结果
    当浏览器不支持Web Speech API时，前端通过MediaRecorder录制音频上传到此接口
    """
    try:
        ...
        if not audio_data:
            return JSONResponse(
                status_code=400,
                content={"success": False, "error": {"code": "E001", "message": "音频数据为空"}}
            )
        ...
        if len(audio_data) > max_size:
            return JSONResponse(
                status_code=400,
                content={"success": False, "error": {"code": "E001", "message": "音频文件过大，最大支持10MB"}}
            )

        content_type = audio.content_type or ""
        logger.info(f"[VoiceAPI] 收到音频: {len(audio_data)} bytes, type={content_type}")

        text = await _process_audio(audio_data, content_type)

        if text:
            return JSONResponse(
                content={"success": True, "data": {"text": text}}
            )
        else:
            return JSONResponse(
                content={"success": True, "data": {"text": ""}, "message": "未能识别语音内容"}
            )

    except Exception as e:
        logger.error(f"[VoiceAPI] 语音识别失败: {e}")
        return JSONResponse(
            status_code=500,
            content={"success": False, "error": {"code": "E003", "message": f"语音识别失败: {str(e)}"}}
        )


async def _process_audio(audio_data: bytes, content_type: str) -> Optional[str]:
    """
    处理音频数据，返回识别文本
    优先尝试使用本地ASR引擎，降级到模拟识别
    """
    try:
        import speech_recognition as sr
        recognizer = sr.Recognizer()

        audio_path = BASE_DIR / "temp_audio"
        suffix = ".webm"
        ...
        temp_file = audio_path.with_suffix(suffix)
        ...
        logger.warning("[VoiceAPI] speech_recognition库未安装，使用模拟识别")
    except Exception as e:
        logger.warning(f"[VoiceAPI] 本地ASR识别失败: {e}，使用模拟识别")

    return _simulate_voice_recognition(audio_data, content_type)


def _simulate_voice_recognition(audio_data: bytes, content_type: str) -> Optional[str]:
    """
    模拟语音识别（当ASR引擎不可用时的降级方案）
    基于音频数据特征生成模拟结果，仅用于演示
    """
    logger.info("[VoiceAPI] 使用模拟语音识别（ASR引擎不可用）")

    audio_hash = hashlib.md5(audio_data[:1024]).hexdigest()
    random.seed(audio_hash)

    demo_results = [
        "塑料瓶", "废电池", "香蕉皮", "旧报纸", "外卖餐盒",
        "易拉罐", "牛奶盒", "碎玻璃", "过期药品", "旧衣服",
        "果皮", "纸箱", "陶瓷碗", "灯泡", "塑料袋"
    ]

    text = random.choice(demo_results)
    logger.info(f"[VoiceAPI] 模拟识别结果: {text}")
    return text
