...
| 指南 | `GET /api/guide/category/{id}` | 单类详细指南 |
| 指南 | `GET /api/guide/confusing` | 易混淆物品列表 |
| 指南 | `GET /api/guide/confusing/{id}` | 单组对比详情 |
| 指南 | `GET /api/guide/item/{keyword}` | 物品详细指引 |
| 历史 | `GET /api/history` | 识别历史列表 |
| 历史 | `DELETE /api/history/{id}` | 删除单条记录 |
...
| 地图 | `GET /api/map/point/{id}` | 单一投放点详情 |
...
| 活动 | `GET /api/activities/{id}/signed` | 检查签到状态 |
| 活动 | `POST /api/activities/{id}/checkin/{uid}` | 活动签到 |
| 活动管理 | `POST /api/activities` | 创建活动 |
| 活动管理 | `PUT /api/activities/{id}` | 更新活动 |
| 活动管理 | `DELETE /api/activities/{id}` | 删除活动 |
| 活动管理 | `POST /api/activities/{id}/cancel` | 取消报名 |
...
├── test_real_images.py        # 真实图片测试
├── verify_integration.py      # 集成验证
├── index.html                 # 根目录索引页
├── pyproject.toml             # Python 项目配置
├── requirements.txt           # Python 依赖
├── package.json               # Node.js（Playwright 测试）
│
├── data/                      # 数据目录
...
默认监听 `http://localhost:8000`，可通过 `uvicorn` 参数自定义端口。

### 访问

浏览器打开 `http://localhost:8000` 即可使用 SPA 前端界面。
...
| opencv-python | (隐式) | 图像特征分析 vector |
