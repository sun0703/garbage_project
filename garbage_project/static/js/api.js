...
class ApiClient {
    ...
    async searchEnhanced(query, topK = 5) {
        if (!query || typeof query !== 'string') {
            throw new ApiError('E001', '搜索关键词不能为空', 400);
        }

        return this.request('GET', `/api/search/enhanced?query=${encodeURIComponent(query.trim())}&top_k=${topK}`);
    }

    async searchSuggest(query, topK = 8) {
        if (!query || typeof query !== 'string') {
            throw new ApiError('E001', '搜索关键词不能为空', 400);
        }

        return this.request('GET', `/api/search/suggest?query=${encodeURIComponent(query.trim())}&top_k=${topK}`);
    }
    ...
    /**
     * 语音识别接口 - 上传音频文件进行语音转文字
     * 当浏览器不支持Web Speech API时，通过MediaRecorder录制音频上传到此接口
     *
     * @public
     * @async
     * @param {Blob} audioBlob - 音频Blob数据
     * @returns {Promise<Object>} 识别结果 { text: string }
     * @throws {ApiError} 音频为空、识别失败等
     *
     * @example
     * const result = await api.voiceToText(audioBlob);
     * console.log(result.text); // '塑料瓶'
     */
    async voiceToText(audioBlob) {
        if (!audioBlob || !(audioBlob instanceof Blob)) {
            throw new ApiError('E001', '音频数据不能为空', 400);
        }

        const formData = new FormData();
        formData.append('audio', audioBlob, 'recording.webm');

        const url = `${this.baseURL}/api/voice/text`;
        const controller = new AbortController();
        ...
        try {
            ...
            try {
                ...
                throw new ApiError('E006', `服务器返回非JSON数据(HTTP ${response.status})`, response.status, response);
            }
            ...
            if (!response.ok) {
                ...
                throw new ApiError(
                    ...
                    mapped ? mapped.message : (serverMsg || `语音识别失败(HTTP ${response.status})`),
                    response.status,
                    response
                );
            }
            ...
            if (err.name === 'AbortError') {
                throw new ApiError('TIMEOUT', '语音识别请求超时', 0, null);
            }

            if (err instanceof TypeError) {
                throw new ApiError('NETWORK', '网络连接失败，请检查后端服务', 0, null);
            }
