...
def set_search_engine(engine) -> None:
    """
    注入搜索引擎实例，供 _get_class_info 的智能匹配使用

    @param engine: SearchEngine 实例
    """
...
def _get_disposal_tips(label_cn: str, class_index: int) -> list[str]:
    ...
    @param label_cn: 物品中文名称
    @param class_index: 垃圾分类索引（0=可回收, 1=有害, 2=厨余, 3=其他）
    @return: 投放建议步骤列表
    """
...
def _get_class_info(
    ...
    @param class_index: 垃圾分类索引（0-3）
    @param is_demo_mode: 是否为演示模式
    @param item_type: 物品类型提示（如 "bottle", "can"）
    @param is_metallic: 是否为金属物品
    @param original_class_name: 原始检测类别名称
    @return: 包含类别名、颜色、图标、投放指导和建议的字典
    """
